...
use awc::{BoxedSocket, error::WsProtocolError, ws::{Codec, Frame, Message}};

use futures::stream::{SplitSink, SplitStream};

use bytes::Bytes;

// type WsFramedSink = SplitSink<Framed<BoxedSocket,Codec>, ws::Message>;
// type WsFramedStream = SplitStream<Framed<BoxedSocket, Codec>>;

pub struct WsClient<T>(pub SinkWrite<T, SplitSink<Framed<T,Codec>, ws::Message>>)
    where
...
impl<T: 'static> StreamHandler<Frame> for WsClient<T>
    where
        T: AsyncRead + AsyncWrite,
{
